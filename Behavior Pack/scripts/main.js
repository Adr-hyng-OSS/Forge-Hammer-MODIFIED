import { world, MinecraftEnchantmentTypes } from "mojang-minecraft";
import { replaceItem } from "./items";
import { setTimeout } from "./timers";
const hammers = ["ihg:wooden_hammer", "ihg:stone_hammer", "ihg:gold_hammer", "ihg:iron_hammer", "ihg:diamond_hammer", "ihg:netherite_hammer"];
const breakableStones = [
    "minecraft:deepslate",
    "minecraft:tuff",
...
function enchantmentEffect(held_item) {
    let enchantments = held_item.getComponent("enchantments").enchantments;
...
function excavateStones(player, held_item, blockLocation) {
    let worldClient = world.getDimension(player.dimension.id);
    for (let y = -1; y < 2; y++) {
        for (let x = -1; x < 2; x++) {
            for (let z = -1; z < 2; z++) {
                let newBlock = worldClient.getBlock(blockLocation.offset(x, y, z));
